	...
	async execute(script: string): Promise<unknown> {
		try {
			...
			const context = vm.createContext({
				console: {
					log: (...args: unknown[]) => core.info(args.join(" ")),
					error: (...args: unknown[]) => core.error(args.join(" ")),
					warn: (...args: unknown[]) => core.warning(args.join(" ")),
					info: (...args: unknown[]) => core.info(args.join(" ")),
				},
				cloudflare,
